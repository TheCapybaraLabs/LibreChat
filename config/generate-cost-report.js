...
const { Transaction, Balance, User } = require('@librechat/data-schemas').createModels(mongoose);
...
(async () => {
  ...
  try {
    const [modelResults, dailyResults, creditResults, balanceResults, totalUsers] =
      await Promise.all([
    ...
    const totalSpent = modelResults.reduce((s, r) => s + Number(r.cost || 0), 0);
    const totalRequests = modelResults.reduce((s, r) => s + Number(r.requests || 0), 0);
