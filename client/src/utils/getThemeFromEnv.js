...
export function getThemeFromEnv() {
  ...
  const buildTheme = (prefix) => {
    ...
    if (getEnv('TEXT_SECONDARY_ALT'))
      theme['rgb-text-secondary-alt'] = getEnv('TEXT_SECONDARY_ALT');
    ...
    if (getEnv('HEADER_BUTTON_HOVER'))
      theme['rgb-header-button-hover'] = getEnv('HEADER_BUTTON_HOVER');

    // Surface colors
    if (getEnv('SURFACE_ACTIVE')) theme['rgb-surface-active'] = getEnv('SURFACE_ACTIVE');
    if (getEnv('SURFACE_ACTIVE_ALT'))
      theme['rgb-surface-active-alt'] = getEnv('SURFACE_ACTIVE_ALT');
    ...
    if (getEnv('SURFACE_PRIMARY_ALT'))
      theme['rgb-surface-primary-alt'] = getEnv('SURFACE_PRIMARY_ALT');
    if (getEnv('SURFACE_PRIMARY_CONTRAST'))
      theme['rgb-surface-primary-contrast'] = getEnv('SURFACE_PRIMARY_CONTRAST');
    if (getEnv('SURFACE_SECONDARY')) theme['rgb-surface-secondary'] = getEnv('SURFACE_SECONDARY');
    if (getEnv('SURFACE_SECONDARY_ALT'))
      theme['rgb-surface-secondary-alt'] = getEnv('SURFACE_SECONDARY_ALT');
    if (getEnv('SURFACE_TERTIARY')) theme['rgb-surface-tertiary'] = getEnv('SURFACE_TERTIARY');
    if (getEnv('SURFACE_TERTIARY_ALT'))
      theme['rgb-surface-tertiary-alt'] = getEnv('SURFACE_TERTIARY_ALT');
    ...
    if (getEnv('BRAND_PRIMARY_HOVER'))
      theme['rgb-brand-primary-hover'] = getEnv('BRAND_PRIMARY_HOVER');
    ...
    if (getEnv('PRIMARY_FOREGROUND'))
      theme['rgb-primary-foreground'] = getEnv('PRIMARY_FOREGROUND');
    if (getEnv('SECONDARY')) theme['rgb-secondary'] = getEnv('SECONDARY');
    if (getEnv('SECONDARY_FOREGROUND'))
      theme['rgb-secondary-foreground'] = getEnv('SECONDARY_FOREGROUND');
    ...
    if (getEnv('DESTRUCTIVE_FOREGROUND'))
      theme['rgb-destructive-foreground'] = getEnv('DESTRUCTIVE_FOREGROUND');
